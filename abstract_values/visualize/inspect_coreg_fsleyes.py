...
Usage:
    python -m abstract_values.visualize.inspect_coreg_fsleyes
    python -m abstract_values.visualize.inspect_coreg_fsleyes --subjects 07 09
    python -m abstract_values.visualize.inspect_coreg_fsleyes --subjects 07 --run 3
"""
...
def run(deriv: Path, subjects: list[str] | None, run_override: int | None):
    if not shutil.which("fsleyes"):
        print(
            ...
            "abstract_values`) or install fsleyes.",
            file=sys.stderr,
        )
        sys.exit(1)
