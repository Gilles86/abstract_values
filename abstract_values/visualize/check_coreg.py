...
Produces one PDF per subject under derivatives/qa/coreg/. Each page shows
the preprocessed T1w with boldref edges overlaid (nilearn's add_edges),
which is the canonical bbregister-style coregistration check.

Run after fmriprep + sync_fmriprep.sh has pulled outputs locally. Example:

    python -m abstract_values.visualize.check_coreg

Specific subjects:

    python -m abstract_values.visualize.check_coreg --subjects 07 08 09 10

Open the resulting PDFs in Preview to flip through quickly.
"""
