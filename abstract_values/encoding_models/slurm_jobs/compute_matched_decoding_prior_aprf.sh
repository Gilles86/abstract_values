#!/bin/bash
#SBATCH --job-name=matched_decoding_prior_aprf
#SBATCH --output=/home/gdehol/logs/matched_decoding_prior_aprf_%j.txt
#SBATCH --ntasks=1
...
# stimulus prior. Same voxel selection / noise-model fit as
# compute_cross_condition_decoding_aprf.py, but compares two priors instead
# of two conditions' tuning curves.
#
# Usage:
#   sbatch --export=PARTICIPANT_LABEL=11 compute_matched_decoding_prior_aprf.sh
#
...
echo "matched_decoding_prior_aprf: sub-${PARTICIPANT_LABEL}  roi=${ROI}  n_voxels=${N_VOXELS}  spherical=${SPHERICAL}"
echo "Args: ${ARGS[*]}"
...
PYTHONUNBUFFERED=1 python -u \
    "$REPO/abstract_values/encoding_models/compute_matched_decoding_prior_aprf.py" \
    "${ARGS[@]}"
